...
function circularTrack(layout,tracks) {
    ...
    if('undefined' !== typeof layout) {
        // Copy over any defaults not passed in by the user
    ...
    this.container = d3.select(layout.container).append("svg")  
    ...
    this.container.on("dblclick", function(d,i) {
        if('undefined' !== typeof this.layout.dblclick) {
            ...
            var curBP = calcRadBPfromXY((d3.mouse(node)[0] - (this.layout.w2)), -(d3.mouse(node)[1] - (this.layout.h2)), xScale)[1];
    ...
    for(var i=0; i < this.tracks.length; i++) {
        if('undefined' !== typeof this.tracks[i].visible) {
            if(! this.tracks[i].visible) { continue; }
        } else {
...
circularTrack.prototype.drawAxis = function() {
    var cfg = this.layout;
    var g = this.g;
    this.axis_container = this.g.append("g").attr("id", "axis_container");
...
circularTrack.prototype.moveAxis = function() {
    ...
    this.axis_container.selectAll("line")
        .data(d3.range(0,cfg.genomesize, cfg.spacing))
        .transition()
        .duration(1000)
        .attr("x1", function(d, i) { return cfg.w2 + (20*Math.cos((d*cfg.radians_pre_bp)-cfg.PI2));})
    ...
    this.axis_container.selectAll("text")
        .data(d3.range(0,cfg.genomesize, cfg.spacing*cfg.legend_spacing))
        .transition()
        .duration(1000)
        .attr("x", function(d, i) { return cfg.w2 + ((cfg.radius+10)*Math.cos((d*cfg.radians_pre_bp)-cfg.PI2));})
...
// Helper function for drawing needed circles such as in stranded tracks
// Can be called standalone in setting up the look of your genome
...
circularTrack.prototype.movePlot = function(i, radius) {
    ...
    var cfg = this.layout;
    var track = this.tracks[i];
    // Save this in case this is a change of radius ratherthan an animated entrance
    if('undefined' !== typeof this.tracks[i].plot_radius) {
        this.tracks[i].plot_radius = radius;
    }
    // We needed to save the new radius but if this track isn't visible, do nothing
    if(! this.tracks[i].visible) {
        return;
    }

    this.tracks[i].plotRange.range([track.plot_radius-(track.plot_width/2), track.plot_radius+(track.plot_width/2)]);
    var plotRange = this.tracks[i].plotRange;
    var lineFunction = d3.svg.line()
...
circularTrack.prototype.drawTrack = function(i, animate) {
    ...
    // set of data
    var arc = d3.svg.arc()
        .innerRadius(function(d){  return (('undefined' == typeof animate) ? calcInnerRadius(track.inner_radius, track.outer_radius, d.strand) : 1); })
        .outerRadius(function(d){  return (('undefined' == typeof animate) ? calcOuterRadius(track.inner_radius, track.outer_radius, d.strand) : 2); })
        .startAngle(function(d){
            if(track.min_slice && (d.end - d.start) < cfg.min_bp_per_slice) {
    ...
    .enter()
        ...
        .attr("class", function(d) { 
            return track.trackName + ('undefined' !== typeof d.strand ? '_' + (d.strand == 1 ? 'pos' : 'neg') : '') + ' ' + ('undefined' !== typeof d.extraclass ? d.extraclass : '');
        })
...
//
////////////////////////////////////////////////
// We will probably need to send a localized version of the data so the update works properly
circularTrack.prototype.drawGlyphTrack = function(i) {
    ...
    for(var i = 0; i < items.length; i++) {
        ...
        xs = (cfg.h/2 + (track.radius*Math.sin((items[i].bp*cfg.radians_pre_bp)-cfg.PI2))) - (cfg.h/2 + (track.radius*Math.sin((items[i-1].bp*cfg.radians_pre_bp)-cfg.PI2)));
        ys = (cfg.h/2 + (track.radius*Math.cos((items[i].bp*cfg.radians_pre_bp)-cfg.PI2))) - (cfg.h/2 + (track.radius*Math.cos((items[i-1].bp*cfg.radians_pre_bp)-cfg.PI2)));
        xs = xs * xs;
        ys = ys * ys;
...
circularTrack.prototype.redrawBrush = function(startRad, endRad) {
    ...
    if('undefined' !== typeof this.brush) {
        this.brushArc.outerRadius(cfg.radius-10);
        this.brush.transition().duration(1000).attr("transform", "translate("+cfg.w2+","+cfg.h2+")");
        this.moveBrush(startRad, endRad);

        d3.select("#brushStart_" + cfg.containerid)     
            .transition()
            .duration(1000)
            .attr("cx", cfg.h + ((cfg.radius-10)*Math.cos(startRad-cfg.PI2)))
            .attr("cy", cfg.h + ((cfg.radius-10)*Math.sin(startRad-cfg.PI2)));
...
circularTrack.prototype.createBrush = function() {
    ...
    var dragStart = d3.behavior.drag().on("drag", function(d) {
        ...
        // THIS IS FOR BRUSH DRAGGING BUT NOT FOR ORIGINAL PLACEMENT
        /////////////////////////////
        g.select("#brushStart_" + cfg.containerid)      
            .attr("cx", function(d, i){
                // SEE NOTE ON BRUSHEND BELOW
                /////////////////////////////
                return (cfg.h/2)/0.8 + (cfg.radius-10)*Math.cos((curRadandBP[0])-cfg.PI2); })
                // return cfg.h/2 + (cfg.radius-10)*Math.cos((curRadandBP[0])-cfg.PI2); })
            .attr("cy", function(d, i){
                return (cfg.h/2) + (cfg.radius-10)*Math.sin((curRadandBP[0])-cfg.PI2); });
    ...
    var dragEnd = d3.behavior.drag().on("drag", function(d) {
        var mx = d3.mouse(this)[0];
        var my = d3.mouse(this)[1];
        var curRadandBP = calcRadBPfromXY(
            (d3.mouse(this)[0] - (cfg.w2)),
            -(d3.mouse(this)[1] - (cfg.h2)),
            xScale
        );
        // Don't allow the brush to go beyond the other
        ...
        g.select("#brushEnd_" + cfg.containerid)        
            .attr("cx", function(d, i){
                // CARLOS MADE CHANGES HERE - BECAUSE I HAD ADJUSTED THE WIDTH I HAD TO READJUST HERE
                // return cfg.h/2 + (cfg.radius-10)*Math.cos((curRadandBP[0])-cfg.PI2);
                return (cfg.h/2)/0.8 + (cfg.radius-10)*Math.cos((curRadandBP[0])-cfg.PI2);
            })
            .attr("cy", function(d, i){
    ...
    this.endBrushObj = g.append("circle")
        .attr({
            ...
            cx: (cfg.w + ((cfg.radius-10)*Math.cos((this.xScale(0))-cfg.PI2))),
            cy: (cfg.h + ((cfg.radius-10)*Math.sin((this.xScale(0))-cfg.PI2))),
            r: 8
        })
        .call(dragEnd);
    ...
    this.startBrushObj = g.append("circle")
        .attr({
            ...
            cx: (cfg.w/2 + ((cfg.radius-10)*Math.cos((this.xScale(0))-cfg.PI2))),
            cy: (cfg.h/2 + ((cfg.radius-10)*Math.sin((this.xScale(0))-cfg.PI2))),
            r: 8
        })
        .call(dragStart);
}
...
circularTrack.prototype.moveBrush = function(startRad, endRad) {
    var g = this.g;
    var cfg = this.layout;
    this.brushArc.startAngle(startRad).endAngle(endRad);
    d3.select('#polarbrush_' + cfg.containerid).attr("d", this.brushArc);
...
circularTrack.prototype.moveBrushbyBP = function(startbp, endbp) {
    ...
    // circularbrush STARTING POINTS HERE
    //////////////////////////////////////
    d3.select("#brushStart_" + cfg.containerid)     
        .attr("cx", (cfg.h/2)/0.8 + ((cfg.radius-10)*Math.cos(startRad-cfg.PI2)))
        .attr("cy", cfg.h/2 + ((cfg.radius-10)*Math.sin(startRad-cfg.PI2)));

    this.brushEnd = endRad;
    this.brushEndBP = endbp;
    d3.select("#brushEnd_" + cfg.containerid)       
        .attr("cx", (cfg.w2)/0.98 + ((cfg.radius-10)*Math.cos(endRad-cfg.PI2)))
        .attr("cy", cfg.h2 + ((cfg.radius-10)*Math.sin(endRad-cfg.PI2)));
}
...
circularTrack.prototype.hideTrack = function(name) {
    var i = this.findTrackbyName(name);
    // We didn't find the track by that name. Is it already not visible? Do nothing
    if(i < 0 || ! this.tracks[i].visible) { return; }
...
circularTrack.prototype.hideGlyphTrackType = function(name, type) {
    var i = this.findTrackbyName(name);
    // We didn't find the track by that name or wrong track type or already visible
    if(i < 0 || this.tracks[i].trackType !== "glyph" || ! this.isvisibleGlyphTrackType(name, type)) {
        return;
    }
...
circularTrack.prototype.isvisibleGlyphTrackType = function(name, type) {
    ...
    // We didn't find the track by that name
    if(i < 0 || this.tracks[i].trackType !== "glyph") {
        return;
    }
...
circularTrack.prototype.dragresize_start = function() {
    ...
    this.drag_shadow.attr("class", "dragbar-shadow");
}
...
circularTrack.prototype.dragresize_end = function() {
    ...
    this.drag_shadow.attr("class", "linear_hidden dragbar-shadow");
    this.resize(newSize);
}
